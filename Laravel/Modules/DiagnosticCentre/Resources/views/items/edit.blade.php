            ...
            <div class="row">
                <div class="form-group col-lg-4 col-md-4 col-sm-12">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" readonly type="text" name="code" id="code" value="{{ old('code', $item->code) }}" required>
                        @if($errors->has('code'))
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('category_id') ? 'is-invalid' : '' }}" name="category_id" id="category_id" required>
                            ...
                            @foreach ($categories as $category)
                                <option value="{{$category->id}}" {{$item->category_id == $category->id ? 'selected' : ""}}><span style="font-weight: bold!important">{{$category->name}}</span></option>
                                @foreach ($category->children as $child)
                                    <option value="{{$child->id}}" {{$item->category_id == $child->id ? 'selected' : ""}}>&nbsp;&nbsp;↦ {{$child->name}}</option>
                                        @foreach ($child->children as $grandchild)
                                            <option value="{{$grandchild->id}}" {{$item->category_id == $grandchild->id ? 'selected' : ""}}>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;↳ {{$grandchild->name}}</option>
                                            @foreach ($grandchild->children as $grandgrandchild)
                                                <option value="{{$grandgrandchild->id}}" {{$item->category_id == $grandgrandchild->id ? 'selected' : ""}}>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;→ {{$grandgrandchild->name}}</option>
                                            @endforeach
                                        @endforeach
                                @endforeach
                            @endforeach
                ...
                <div class="form-group col-lg-3 col-md-3 col-sm-12">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="text" name="exp_date" value="{{ old('exp_date', $item->exp_date) }}" autocomplete="off" class="datepicker-here form-control digits">
                        </div>
                ...
                <div class="form-group col-lg-3 col-md-3 col-sm-12">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="text" name="price" id="price" value="{{ old('price', $item->price) }}" required>
                        @if($errors->has('price'))
                ...
                <div class="form-group col-lg-3 col-md-3 col-sm-12">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('offer_price') ? 'is-invalid' : '' }}" type="text" name="offer_price" id="offer_price" value="{{ old('offer_price', $item->offer_price) }}" >
