            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-Permission">
                ...
                <tbody>
                    ...
                    @foreach($paids as $key => $paid)
                        <tr data-entry-id="{{ $paid->id }}">
                            <td>{{ $paid->bill->code ?? '' }}</td>
                            <td>{{ $paid->net_payable ?? 0 }}</td>
                            <td>{{ $paid->bill->discount ?? 0 }}</td>
                            <td>{{ $paid->bill->tax ?? 0 }}</td>
                            <td>{{ $paid->received_amount ?? 0 }}</td>
                            <td>{{ Carbon\Carbon::parse($paid->created_at)->format($settings['date_format'] ?? 'j F, Y') }}</td>
                            <td>
                                @can('diagnostic_report_templates_show')
                                    <a type='button' class='btn btn-sm btn-primary' href="{{ route('diagnostic-centre.pathology-bills.invoice', $paid->bill->id) }}"><i class='far fa-eye'></i></a>
                                @endcan
                            </td>
                        </tr>
                        @php
                        $total_net_payable += $paid->net_payable;
                        $total_discount += $paid->bill->discount;
                        $total_tax += $paid->bill->tax;
                        $total_received_amount += $paid->received_amount;
                    @endphp
                    @endforeach
                    @if (count($paids) > 0)
                        <tr>
